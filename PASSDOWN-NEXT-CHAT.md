...
**Last updated:** 2026-04-24 — **`main` at 6.0.15** (see `package.json` / **CHANGELOG**). **6.0.15** — **Secondary** → **Streaming** strip only: **5** first, then **9→20** with delay (main **Streaming** page still **4→20**). **6.0.14** — Indian+TV+**Netflix** only: `with_origin_country=IN` on discover; **Prime/Hulu** broad+client. **6.0.12** — Indian taste = language or **`IN`** origin. **6.0.9** — `SECONDARY_AVAILABILITY_TMDB_REGION` = **US**; passdown: **All services** on secondary still *thin* (TV **`fetchStreamingTVForMarket`** = new/7d/trending gate **before** Indian filter) — **user will implement** pool/combine in **next chat** (see **§ Open / follow-ups — Last session**). **Circles:** **§ Master backlog 4a–4e**; **Deferred** scroll-stop **`predict_cached`**.

---

## Tell the next chat (copy from here)

> Cinematch is on **`main` at 6.0.15** (see `package.json`). Read **`@PASSDOWN-NEXT-CHAT.md`** and follow **`.cursor/rules/cinematch-discussion-first.mdc`** and **`.cursor/rules/cinematch-handoff.mdc`** (don’t code unless I say *code now* / *implement* / *fix* / *do it* for that task, unless I clearly ask for code in the same message). On **handoff updates**, include the session’s **last note** in passdown (see **§ For the assistant** item 4).  
> **Context:** **6.0.11–6.0.12** — secondary: **no** profile **genres** on secondary shelves/merge; **Indian** = broad US + **client** taste (lang **or** **`IN`** origin). **6.0.15** — **Secondary** → **Streaming** only: **5**-then-stagger; **6.0.9** `SECONDARY_AVAILABILITY_TMDB_REGION` = **US**; per-provider: **6.0.14** Netflix **TV** = `with_origin_country=IN`; Prime/Hulu **TV** = broad+client. **Next implementation (user):** **All services** on **secondary** still reads small pools for **Series** ( **`fetchStreamingTVForMarket`** = new/7d/trending **before** Indian filter) — see **§ Open / follow-ups** / **`App.jsx`**. **Main Streaming** page unchanged for stagger (still 4). **Your Picks** = profile. **6.0.0** **`App.css`**. **Circles** / **Vercel** / **cron** — full passdown. **Backlog** — **§ Master backlog**.
...
| Item | State |
|------|--------|
| **App version** | **6.0.15** (`package.json` / `CHANGELOG.md`); **Cinemastro v…** = **`APP_VERSION`**. **6.0.15** — **Secondary** → **Streaming** strip: **5**-then-stagger to **20**; **6.0.14** Netflix+Indian+TV: `with_origin_country=IN`; **6.0.9** US `watch_region`; **6.0.0** **`App.css`**. |
| **Git** | **`main`** = **6.0.15** (confirm **`origin`**) — **3ff746e**+ if newer; **5.6.52** edit circle, **5.6.51** **`creator_leave_circle`**, **5.6.50** Forward. |
...
- **User intent (next chat — implement in repo):** On **secondary Region** → **Streaming** with **All services**, **movies/series** lists stay **small** because **`fetchStreamingTVForMarket`** (and to a degree **`fetchStreamingMoviesForMarket`**) build a **tight** pool: TV keeps only **new series (180d)** / **new season (7d air)** / **trending**, **then** Indian taste — so few rows; **6.0.15** stagger only **reveals** from that pool, it does not add titles. **Planned work:** **combine** / **align** All-services with **per-provider**-style **discover** (or **relax** the TV **new/7d** gate for Indian secondary), **dedupe**, same Indian taste, keep **5→20** stagger on **this screen only**. Primary code: **`App.jsx`** — `fetchStreamingTVForMarket`, `fetchStreamingMoviesForMarket`, `secondaryActiveRawRows` / all-services `useEffect` for `secondaryRegionAllServicesStreamDisplayLen` (6.0.15).
- **Shipped to `main` (through 6.0.15):** **6.0.9–6.0.12** US + Indian taste; **6.0.11** no profile **genres** on secondary; **6.0.14** **Netflix** TV = `with_origin_country=IN`, **Prime/Hulu** broad; **6.0.15** **Secondary** **Streaming** strip: **5** then **9,14,19,20**; main **Streaming** page still **4**-start reveal.
- **Prod Supabase (stable):** hand-paste vs **`schema_migrations`** — **§ Snapshot** for migrations; **`creator_leave_circle`**, etc.
- **Open (elsewhere):** **§ Master backlog** 4a–4c, 4e; **4d** disintegrate TBD. **Deferred:** scroll-stop **`predict_cached`**. **Beta** feedback.
