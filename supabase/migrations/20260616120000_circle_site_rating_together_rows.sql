-- Circle RPCs: attach `site_rating` (Cinemastro global avg) for every strip/grid row where
-- `get_cinemastro_title_avgs` returns a score — not only `section = 'solo'`. Together rows now
-- receive the same global average as solo. Client falls back to TMDB for the gold under-title
-- number when `site_rating` is null (cold on Cinemastro).

-- Recent strip: order by max(greatest(ratings.rated_at, share.created_at)) per title in this circle
-- so forwarding a title (new rating_circle_shares row) surfaces it as “recent” there, not only on re-rate.
