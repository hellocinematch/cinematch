-- Circle detail grids: "All" (every title rated by a member, paginated by last activity)
-- and "Top" (highest circle average, ties: more raters, then more recent; hard cap 25 titles).
...
comment on function public.get_circle_rated_all_grid(uuid, int, int) is
  'Circle grid: all rated titles, last activity desc, paginated.';
...
comment on function public.get_circle_rated_top_grid(uuid, int, int) is
  'Circle grid: top circle averages (max 25 titles), paginated.';
