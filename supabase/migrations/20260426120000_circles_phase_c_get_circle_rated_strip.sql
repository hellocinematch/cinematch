-- Circles Phase C — backend strip aggregation (RPC only; predictions run in get-circle-rated-titles Edge).
-- Display contract: supabase/migrations/20260422120000_circles_schema.sql (PHASE C DISPLAY CONTRACT).
--
--  * Ensures public.ratings.rated_at exists for archived-circle cutoff (rated_at < circles.archived_at).
--  * Adds get_circle_rated_strip(circle_id) → jsonb (member_count, gated, titles[]).
--    SECURITY DEFINER + auth.uid() membership check. Callable only as authenticated (JWT).

-- ------------------------------------------------------------------------------------------------
-- ratings.rated_at (archive cutoff for circle strip)
-- ------------------------------------------------------------------------------------------------
...
begin
  if not exists (
    ...
    alter table public.ratings add column rated_at timestamptz not null default now();
  end if;
end
$mr$;

comment on column public.ratings.rated_at is
  'When the user rated this title. Used for circles archive cutoff: ratings after circles.archived_at are excluded from the circle strip.';
...
begin
  ...
  base as (
    ...
    where (v_archived_at is null or r.rated_at < v_archived_at)
  ),
  agg as (
    select
      b.media_type,
...
comment on function public.get_circle_rated_strip(uuid) is
  'Phase C circle strip: circle_members ∩ ratings, archive cutoff, two sections (together|solo), site avg for solo via get_cinemastro_title_avgs. viewer_score = caller''s rating if any. Caller must be a member; gated when member_count < 2.';
