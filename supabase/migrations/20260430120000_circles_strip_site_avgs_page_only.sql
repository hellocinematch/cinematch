-- Perf: site-wide averages for solo rows were computed for *every* solo title in the circle
-- via get_cinemastro_title_avgs(solo_payload), which is O(solo titles). Ordering only needs
-- section + last_at; site_rating is display-only — compute avgs for the current page slice only.
...
comment on function public.get_circle_rated_strip(uuid, int, int) is
  'Phase C circle strip (page-only site avgs): paginated, max 20 rows; total_eligible + has_more.';
