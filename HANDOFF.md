...
3. **Version bump rule:** trust **`package.json`** / **`CHANGELOG.md`** for the current release (now **5.6.7**). Bump both whenever you ship product code; add **`CHANGELOG.md`** in the **same release commit** as the first shipping change—not in a handoff-only or docs-only commit.
...
- **`package.json` version:** **`5.6.7`** — see **`CHANGELOG.md`**; circle feeds use **`rating_circle_shares`**.
...
Expected version line: match **`package.json`** / **`CHANGELOG.md`** (currently **`5.6.7`**).
